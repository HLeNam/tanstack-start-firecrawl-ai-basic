...
export const saveSummaryAndGenerateTagsFn = createServerFn({
  ...
  .handler(async ({ context, data }) => {
    const existing = await prisma.savedItems.findUnique({
      where: {
        id: data.id,
        userId: context.session.user.id,
      },
    ...
    const { text } = await generateText({
      ...
      Extract 3-5 short, relevant tags that categories the content.
    ...
    const items = await prisma.savedItems.update({
      where: {
        id: data.id,
        userId: context.session.user.id,
      },
